...
class NaverMapViewController extends GetxController {
  ...
  // /// **loadMultiWaypointPathData**
  // ///
  // /// 경유지를 포함한 전체 경로를 로드하는 메서드입니다.
  // ///
  // /// - **매개변수**:
  // ///   - `waypoints` (List<LatLng>): 출발지, 경유지들, 도착지 순서의 좌표 리스트
  // ///   - `chooseRoute` (String): 경로 옵션
  // ///
  // /// - **사용 예시**:
  // /// ```dart
  // /// final waypoints = [
  ...
  // /// ```
  // Future<void> loadMultiWaypointPathData(
  //   List<LatLng> waypoints,
  //   String chooseRoute,
  // ) async {
  //   try {
  //     debugPrint('경유지를 포함한 경로 로드 시작 - 총 ${waypoints.length}개 지점');
      
  //     // 경유지를 포함한 전체 경로 데이터 가져오기
  //     final result = await apiService.fetchMultiWaypointPath(
  //       waypoints: waypoints,
  //       chooseRoute: chooseRoute,
  //     );

  //     debugPrint('경유지 경로 로드 완료:');
  //     debugPrint('- 총 거리: ${result.totalDistance}m');
  //     debugPrint('- 총 시간: ${result.totalTime}초');
  //     debugPrint('- 총 포인트: ${result.paths.length}개');
  //     debugPrint('- 구간 수: ${result.segmentDetails.length}개');

  //     // 브랜치 정보 업데이트 (각 브랜치 간의 방향 계산)
  //     for (int i = 0; i < result.branchInfo.length - 1; i++) {
  //       double newBearingValue = calculateBearing(
  //         result.branchInfo[i].point.latitude,
  //         result.branchInfo[i].point.longitude,
  //         result.branchInfo[i + 1].point.latitude,
  //         result.branchInfo[i + 1].point.longitude,
  //       );
  //       result.branchInfo[i].bearingToPoint = newBearingValue;
  //     }
  ...
  //         await mapController!.clearOverlays(type: NOverlayType.marker);
  //         // 경유지 마커 추가
  //         addWaypointOverlays(result.paths, waypoints);
  //       } catch (e) {
  //         debugPrint('지도 오버레이 업데이트 실패: $e');
  //         // 오버레이 실패해도 네비게이션은 계속 진행
  //       }
  //     }
  ...
  //     // 경로 정보 음성 안내
  //     final minutes = (result.totalTime / 60).round();
  //     final kilometers = (result.totalDistance / 1000).toStringAsFixed(1);
  //     speakText("경유지를 포함한 총 ${kilometers}킬로미터, 약 ${minutes}분 경로로 안내를 시작합니다.");

  //   } catch (e) {
  //     debugPrint('경유지 경로 로드 실패: $e');
  //     String errorMessage = "경로를 불러오는데 실패했습니다.";
      
  //     if (e.toString().contains('MissingPluginException')) {
  //       errorMessage = "지도 플러그인 오류가 발생했습니다. 앱을 다시 시작해주세요.";
  ...
  // /// **addWaypointOverlays**
  // ///
  // /// 경유지를 포함한 경로 오버레이를 지도에 추가합니다.
  // ///
  // /// - **매개변수**:
  // ///   - `paths` (List<LatLng>): 전체 경로 좌표
  // ///   - `waypoints` (List<LatLng>): 경유지 좌표들
  // void addWaypointOverlays(List<LatLng> paths, List<LatLng> waypoints) async {
  ...
  //         // navigatorKey.currentContext가 null일 수 있으므로 체크
  ...
  //             NMarker(
  //               id: 'waypoint_$i',
  //               position: NLatLng(waypoint.latitude, waypoint.longitude),
  //               icon: iconImage,
  //             ),
  //           );
  //         }
  //       } catch (e) {
  //         debugPrint('경유지 마커 $i 생성 실패: $e');
  //         // 개별 마커 실패해도 계속 진행
  //       }
  //     }
  ...
  //     }
  //   } catch (e) {
  //     debugPrint('경유지 오버레이 추가 실패: $e');
  //   }
  // }

  // /// **startNavigationWithWaypoints**
  // ///
  // /// 경유지를 포함한 네비게이션을 시작합니다.
  // ///
  // /// - **매개변수**:
  // ///   - `waypoints` (List<LatLng>): 출발지, 경유지들, 도착지 순서의 좌표 리스트
  // ///   - `routeOption` (String): 경로 옵션 (선택사항, 기본값: "0")
  // ///
  // /// - **사용 예시**:
  // /// ```dart
  // /// final waypoints = [
  ...
  // ///   LatLng(37.55279, 126.92432), // 도착
  // /// ];
  // /// await controller.startNavigationWithWaypoints(waypoints, "0");
  // /// ```
  // Future<void> startNavigationWithWaypoints(
  //   List<LatLng> waypoints, {
  //   String? routeOption,
  // }) async {
  //   if (waypoints.length < 2) {
  //     debugPrint('경유지 네비게이션: 최소 2개의 지점이 필요합니다.');
  //     speakText("출발지와 도착지를 모두 설정해주세요.");
  //     return;
  //   }

  //   try {
  //     debugPrint('경유지 네비게이션 시작 - 총 ${waypoints.length}개 지점');
  //     final selectedRoute = routeOption ?? choose_route.value;
  //     await loadMultiWaypointPathData(waypoints, selectedRoute);
  //   } catch (e) {
  //     debugPrint('경유지 네비게이션 시작 실패: $e');
  //     speakText("경유지 네비게이션을 시작할 수 없습니다.");
  //   }
  // }

  // /// **예제 사용법 메서드**
  // ///
  // /// 경유지 기능을 테스트하기 위한 예제 메서드입니다.
  // Future<void> testWaypointNavigation() async {
  //   // 예시 좌표들 (사용자가 제공한 좌표)
  //   final waypoints = [
  //     LatLng(37.55677, 126.92365), // 출발
  //     LatLng(37.55395, 126.92775), // 경유지1
  ...
  //   debugPrint('=== 경유지 네비게이션 테스트 시작 ===');
  //   await startNavigationWithWaypoints(waypoints);
  // }
