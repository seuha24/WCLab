...
class _NaverMapViewState extends State<NaverMapView> {
  ...
  Future<void> positionUpdate(Duration sensorInterval) async {
    ...
    // 속도 제한
    const List<double> walkSpeed = [0.833, 0.972, 1.111, 1.25]; // 3.0km/h, 3.5km/h, 4.0km/h, 4.5km/h
    double maxSpeed = walkSpeed[3]; // 보행 속도
    velocityX = velocityX > maxSpeed ? maxSpeed : velocityX;
    velocityY = velocityY > maxSpeed ? maxSpeed : velocityY;
    ...
    int moveIndex(List<BranchInfo> currentWindow) {
      ...
      for (BranchInfo branchInfo in currentWindow) {
        currentDistance = calculateDistance(branchInfo.point.latitude,
            branchInfo.point.longitude, finalLatitude, finalLongitude);
                ...
                NaverMap(
                  options: NaverMapViewOptions(
                    ...
                    initialCameraPosition: NCameraPosition(
                      target: NLatLng(finalLatitude, finalLongitude),
                      zoom: 18.5, // 지도의 확대 정도
                  ...
                  onMapReady: (controller) {
                    ...
                    _updateCurrentLocationMarker(finalLatitude, finalLongitude);
                    _updateMapPosition(
                        finalLatitude, finalLongitude, compassValue);
                  },
                ...
                Positioned(
                  ...
                  child: Column(
                    children: [
                      ...
                      Gap(height: AppSizes.scaledHeight(10)),
                      Container(
                        decoration: BoxDecoration(
                          color: const Color.fromARGB(255, 190, 164, 164),
                          borderRadius: BorderRadius.circular(10),
                        ...
                        child: GestureDetector(
                          onTap: () async {
                            GeoLocation? newSelectedLocation =
                                await Navigator.push(
                              context,
                              PageRouteBuilder(
                                pageBuilder: (context, animation,
                            ...
                            if (newSelectedLocation != null) {
                              ...
                              if (isStart == true) {
                                await _getGeometry(
                                    ...
                                    startSelectedLocation!
                                        .lng); // 새로운 목적지로 지도 업데이트
                              } else if (isStart == false) {
                                await _getGeometry(finalLatitude,
                                    finalLongitude); // 새로운 목적지로 지도 업데이트
                              }

                              // 주기적으로 Timer를 실행하기 전에 먼저 방향값을 초기화 해준다.
                              // branchinfo 배열을 순회하면서 bearingTobranch 값을 변경합니다.
                              for (int i = 0;
                                  i < branchinfo.length - 1;
                                  i++) {
                                // 변경할 값으로 갱신합니다.
                              ...
                              Timer.periodic(Duration(seconds: 2),
                                ...
                                if (isStart == false) {
                                  if (branchinfo.isNotEmpty &&
                                    ...
                                    while (branchTargetIndex <
                                            branchinfo.length &&
                                        !branchinfo[branchTargetIndex]
                                            .branch) {
                                      branchTargetIndex++;
                                    }
                                    if (branchinfo[branchTargetIndex]
                                        .branch) {
                                  ...
                                  if (remainDistance < 0.015) {
                                    // 만약 그 목표 지점이 횡단보도라면
                                    if (branchinfo[currentIndex]
                                            .crosswalk ==
                                        true) {
                                      // 경광등을 켜라.
                                      final result =
                                          await flashOnWithWeather(
                                              NoParams());
                                      if (result.isLeft()) {
                                    ...
                                    if (branchinfo[targetIndex].branch ==
                                        true) {
                                      await tts.speak(
